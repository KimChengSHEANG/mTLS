...
def run_tuning(params):
    ...
    test_set = pd.read_csv(RESOURCES_DIR / 'mask_pred_candidates/lex.mturk_topk_10_roberta-base.csv')
    test_set = preprocessor.preprocess_valid_or_test_set(test_set, use_mask_pred_candidates=False)
    
    return evaluation_pruning(test_set, 'lexmturk', features_kwargs, phase=phase, lang=lang, model_dirname=model_name)
